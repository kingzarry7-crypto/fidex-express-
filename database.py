...
def get_connection():
    # Dynamically fetch the connection string on each function invocation
    db_url = (
    ...
    if not db_url:
        raise ValueError(
            "Database connection URL is missing. Ensure Neon integration environment variables are bound on Vercel."
        )

    # Ensure SSL mode is enabled for Neon/Vercel Cloud Postgres
